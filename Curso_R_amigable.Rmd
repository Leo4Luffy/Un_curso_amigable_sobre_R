...
```{r ejemplo_5_2_1_6_1, exercise = TRUE}
...
Para cambiar el nombre de los valores de las columnas cualitativas, se puede usar la función `recode_factor()` dentro de la función `mutate()`.
...
Si desea convertir una columna numérica en una columna cualitativa, puede hacer uso de la función `case_when()`.
...
<!--tratar de entender a http://www.rebeccabarter.com/blog/2019-01-23_scoped-verbs/ para incluir mas procesos con la función mutate()-->
...
#### 5.2.1.8 Resumir valores con `group_by()` y `summarise()`
...
resumen <- data.frame(
  ...
  annotate(geom = 'text', x = 28.5, y = 4.75, label = 'MALE', family = 'gochi', size = 7.4, colour = 'black') +
  #annotate(geom = 'rect', xmin = 12.2, xmax = 13.5, ymin = 11.72, ymax = 11.78, colour = 'red', fill = 'red', alpha = 0.2) +
  annotate(geom = 'text', x = 17.0, y = 11.75, label = 'Grupos <- group_by(.data = penguins, species)', size = 7.4, colour = 'black') +
  annotate(geom = 'text', x = 19.4, y = 11.25, label = 'summarise(.data = Grupo, mean(culmen_length), mean(culmen_depth))', size = 7.4, colour = 'black') +
  scale_y_continuous(limits = c(4.0, 11.78)) +
  #scale_x_continuous(limits = c(8, 5.0)) +
  theme_void() +
  theme(axis.text = element_blank(),
...
```{r ejemplo_5_2_1_8_1, exercise = TRUE}

Grupos <- group_by(.data = penguins, species)

summarise(.data = Grupos, mean(culmen_length_mm), mean(culmen_depth_mm))
```

#### 5.2.1.9 Combinando funciones con `%>%`
