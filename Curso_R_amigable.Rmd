...
Ya en este punto de seguro se preguntará ¿es posible ir más alla de lo que hasta ahora hemos visto? La respuesta es definitivamente un si. Un ejemplo de ello es la famosa visualización de [Hans Rosling](https://www.ted.com/talks/hans_rosling_the_best_stats_you_ve_ever_seen) donde representaba la población mundial, la expectativa de vida y varios indicadores económicos mediante una visualización animada.

Afortunadamente, hacer este tipo de visualizaciones se ha vuelto hoy en día bastante simple mediante el uso de paquetes de `R`. Para explicar como hacer estas visualizaciones se empleara el conjunto de datos `gapminder`. A continuación podrá hechar un vistazo de la estructura de este conjunto de datos :
...
1. En `country` podrá encontrar el nombre del país.
2. En `continent` podrá encontrar el nombre del continente del respectivo país.
3. En `year` podrá encontrar el año en el que hubo el registro.
4. En `lifeExp` se describe la expectativa de vida al nacer en el respectivo año, continente y país.
5. En `popular` se  describe la población total en el respectivo año, continente y país.
6. En `gdpPercap` se describe el PIB per cápita en el respectivo año, continente y país.
...
Esto lo hace al proporcionar una gama de nuevas clases de gramática o funciones que se pueden agregar al código de graficación y personalizar de este modo como la gráfica debe cambiar con el tiempo. Estas nuevas funciones son:
  </div>
  <div class="col-md-6" markdown="1">
```{r, echo = FALSE, eval = TRUE, out.width = "70%", fig.align = "center"}
