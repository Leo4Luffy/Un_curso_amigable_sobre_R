...
Tuberculosis %>%
  download_this(
    output_name = 'Tuberculosis',
    output_extension = '.xlsx',
    button_label = 'Descargar datos Tuberculosis',
    button_type = 'warning',
    has_icon = TRUE,
    icon = 'fa fa-download'
  )
...
palmerpenguins::penguins %>%
  download_this(
    ...
    output_extension = '.xlsx',
    button_label = 'Descargar datos penguins',
    button_type = 'warning',
    has_icon = TRUE,
    icon = 'fa fa-download'
  )
...
pokemon %>%
  download_this(
    output_name = 'pokemon',
    output_extension = '.xlsx',
    button_label = 'Descargar datos pokemon',
    button_type = 'warning',
    has_icon = TRUE,
    icon = 'fa fa-download'
  )
...
gapminder::gapminder %>%
  download_this(
    output_name = 'gapminder',
    output_extension = '.xlsx',
    button_label = 'Descargar datos gapminder',
    button_type = 'warning',
    has_icon = TRUE,
    icon = 'fa fa-download'
  )
...
## 7. Una breve introducción al diseño de experimentos

<!--La elección del diseño de un estudio es una de las etapas más complejas en el proceso de investigación, una vez se debe tomar en consideración una seríe de hechos como:

- La información previa que existe respecto al tema de investigación.
- La factibilidad de realizar la investigación.

A todo lo anterior se le suma la decisión por parte del investigador de si se mantendrá al margen del desarrollo de los acontecimientos o si decidirá intervenir en él. De esta forma, es posible establecer la división de los diferentes estudios que existen en dos tipos:

```{r, echo = FALSE, eval = TRUE, out.width = "40%", fig.align = "center"}

knitr::include_graphics("images/61va_imagen.png")  
```

### 7.1 Estudios experimentales

Los estudios experimentales se caracterizan por el hecho de que el investigador interviene y controla a voluntad las variables consideradas como causales de un fenómeno, permitiendo de esta forma evaluar una respuesta bajo diferentes configuraciones.

```{r, echo = FALSE, eval = TRUE, out.width = "56%", fig.align = "center"}

knitr::include_graphics("images/62va_imagen.png")  
```

+ Tratamiento

Son las condiciones cuyo efecto se evalúa a través de una respuesta (aquí, el agua sería el tratamiento y el crecimiento de la semilla la respuesta).

+ Grupo control y experimental

En un experimento pueden presentarse dos grupos: el grupo experimental sobre el cual se aplica el tratamiento (aquí, las semillas regadas) y el grupo control que al ser comparados con el grupo experimental permiten determinar si hubo o no efecto del tratamiento (aquí, las semillas sin riego).

+ Variable dependiente e independiente

El elemento que es diferente entre los grupos control y experimental (en este caso, la cantidad de agua) se conoce como variable independiente. Por otra parte, la variable dependiente (en este caso, las semillas que crecen) es el elemento que se mide para ver si el tratamiento tuvo efecto. 

+ Datos experimentales

Son las observaciones realizadas durante el experimento (aquí, el número de semillas de la planta de frijol que crecieron en cada maceta después de una semana).

+ Repetición

Son cada uno de los elementos del grupo experimental que fueron sometidos a un mismo tratamiento (aquí, las diez semillas regadas con agua).

```{r, echo = FALSE, eval = FALSE, out.width = "44%", fig.align = "center"}

knitr::include_graphics("images/63va_imagen.png")  
```

+ Aleatorización

Es la asignación al azar de los tratamientos a los grupos experimentales.

+ Hipótesis

<!--ver en https://bookdown.org/ndphillips/YaRrr/a-short-introduction-to-hypothesis-tests.html y explicarlo al momento de formularlo en el anova... aquí, tambien explican las estadísticas de prueba-->

<!--#### 7.1.1 Análisis de varianza

El __análisis de varianza__ es un procedimiento estadístico que compara el valor promedio de varios grupos experimentales y determina si son significativamente diferentes entre sí.

El objetivo del análisis de varianza es probar la siguiente hipótesis de igualdad de los tratamientos con repecto al valor promedio de la respectiva variable dependiente:
...
el cual se puede escribir en forma equivalente como:

\begin{equation*}
\begin{split}
&H_{0}: \tau_{1} = \tau_{2} = ... = \tau_{k} = 0\\
&H_{a}: \tau_{i} \neq 0
\end{split}
\end{equation*}
...
knitr::include_graphics("images/64va_imagen.png")  
```

El anterior juego de hipótesis permite plantear el siguiente modelo estadístico en que se basa el análisis de varianza:

\begin{equation*}
x_{ij} = \mu + \tau_{i} + \epsilon_{ij} \left\{\begin{matrix} i = 1, 2, ..., k
\\ j = 1, 2, ..., r\end{matrix}\right.
\end{equation*}
...
ggplot() +
  geom_circle(data = circulos, aes(x0 = x, y0 = y, r = 1.5, fill = grupo, colour = grupo), alpha = 0.1, size = 1.0) +
  annotate(geom = 'text', x = -0.4, y = 1.3, label = 'Variabilidad', family = 'gochi', size = 6.4, colour = 'black') +
  annotate(geom = 'text', x = -0.4, y = 1.0, label = 'debida a los', family = 'gochi', size = 6.4, colour = 'black') +
  annotate(geom = 'text', x = -0.4, y = 0.7, label = 'tratamientos', family = 'gochi', size = 6.4, colour = 'black') +
  annotate(geom = 'text', x = 1.6, y = 1.3, label = 'Variabilidad', family = 'gochi', size = 6.4, colour = 'black') +
  annotate(geom = 'text', x = 1.6, y = 1.0, label = 'debida al', family = 'gochi', size = 6.4, colour = 'black') +
  annotate(geom = 'text', x = 1.6, y = 0.7, label = 'error', family = 'gochi', size = 6.4, colour = 'black') +
  annotate(geom = 'segment', x = 1.0, xend = 1.0, y = -0.5, yend = 2.5, colour = 'black', size = 1.2, linetype = 'dashed') +
  annotate(geom = 'text', x = 4.75, y = 1.3, label = 'Variabilidad', family = 'gochi', size = 6.4, colour = 'black') +
  annotate(geom = 'text', x = 4.75, y = 1.0, label = 'debida a los', family = 'gochi', size = 6.4, colour = 'black') +
  annotate(geom = 'text', x = 4.75, y = 0.7, label = 'tratamientos', family = 'gochi', size = 6.4, colour = 'black') +
  annotate(geom = 'text', x = 5.9, y = 1.3, label = 'Variabilidad', family = 'gochi', size = 6.4, colour = 'black') +
  annotate(geom = 'text', x = 5.9, y = 1.0, label = 'debida al', family = 'gochi', size = 6.4, colour = 'black') +
  annotate(geom = 'text', x = 5.9, y = 0.7, label = 'error', family = 'gochi', size = 6.4, colour = 'black', angle = -0.35) +
  annotate(geom = 'segment', x = 5.35, xend = 5.35, y = -0.5, yend = 2.5, colour = 'black', size = 1.2, linetype = 'dashed') +
  theme_void() +
  theme(legend.position = 'none') +
  scale_fill_manual(values = c('cyan', 'yellow')) +
  scale_colour_manual(values = c('cyan', 'yellow')) +
  theme(axis.text = element_blank(),
        axis.ticks = element_blank(),
        axis.title = element_blank())
```
...
<!--donde:
$x_{ij}$: j-ésima repetición del i-ésimo tratamiento;
$\mu$: promedio general;
$\tau_{i}$: efecto del i-ésimo tratamiento ($\mu_{i} - \mu$);
$\epsilon_{ij}$: desviación de la j-ésima repetición del i-ésimo tratamiento respecto a la media, $\mu_{i}$, del i-ésimo tratamiento.-->

<!--
El análisis de varianza es un procedimiento que permite comparar el valor promedio de varios grupos experimentales, mediante la partición de la variabilidad total en diferentes fuentes.-->
<!--
```{r, echo = FALSE, eval = TRUE, out.width = "88%", fig.align = "center"}

knitr::include_graphics("images/anova.png")  
```

<section style="text-align: center;"><small>Ilustracíon modificada de <font color = "black"><u>\@YaRrrBook</u></font>.</small></section>
