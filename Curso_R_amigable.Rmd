...
:::caution
Como se pudo observar, la función `pivot_longer()` hace lo opuesto a `pivot_wider()`. Estas son funciones complementarias, es decir, si al resultado de aplicar la función `pivot_wider()` le aplicamos la función `pivot_longer()` se llega al conjunto de datos original. Otra observación interesante es que `pivot_longer()` alarga los conjuntos de datos, mientras que `pivot_wider()` los hace más anchos.
:::
