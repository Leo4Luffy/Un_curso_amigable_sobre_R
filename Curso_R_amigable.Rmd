...
Para asociar el gráfico a un conjunto de datos en específico se emplea el argumento `data`. Luego se define un mapeo haciendo uso de la función `aes()` dentro de `ggplot()`, lo cual, permite seleccionar las variables a graficar y cómo deben ser presentadas. Con `geoms` por último se indica cómo se representaran los datos en el gráfico.
...
ggplot(
  data = pokemon,
  mapping = aes(x = ataque, y = defensa)
  ) +
  geom_point()
```
