...
<div class="row">
  ...
  Tratamiento<HR>
  Son las condiciones cuyo efecto se evalúa a través de una respuesta.
  </div>
  </div>
  <div class="col-md-6" markdown="1">
  <div class="box-red-2">
  Grupo control y experimental<HR>
  El grupo experimental es sobre el cual se aplica el tratamiento y el grupo control es aquel que al ser comparado con el grupo experimental determina si hubo o no efecto del tratamiento.
  </div>
  </div>
</div>
...
<div class="row">
  <div class="col-md-6" markdown="1">
  <div class="box-gray-2">
  Variable dependiente e independiente<HR>
  La variable dependiente es el elemento que es diferente entre los grupos control y experimental, mientras que la variable dependiente es el elemento que se mide para ver si el tratamiento tuvo efecto.
  </div>
  </div>
  <div class="col-md-6" markdown="1">
  <div class="box-yellow-2">
  Datos experimentales<HR>
  Son las observaciones realizadas durante el experimento.
  </div>
  </div>
</div>
...
<div class="row">
  ...
  Repetición<HR>
  Son cada uno de los elementos del grupo experimental que son sometidos a un mismo tratamiento.
  </div>
  </div>
  <div class="col-md-6" markdown="1">
  <div class="box-grey-2">
  Variabilidad<HR>
  Hace referencia a las distintas causas por las que los datos experimentales obtenidos tienen distintos valores. Esta variabilidad puede ser de dos tipos: debido a los tratamientos y debido al error.
  </div>
  </div>
</div>
...
<div class="row">
  ...
  <div class="box-red-2">
  Hipótesis<HR>
  Estas hipótesis generalmente son de dos tipos: la hipótesis nula ($H_{0}$) y la hipótesis alterna ($H_{a}$). 
  </div>
  </div>
</div>
...
\begin{equation*}
y_{ij} = \mu + \tau_{i} + \epsilon_{ij} \left\{\begin{matrix} i = 1, 2, ..., k
\\ j = 1, 2, ..., r\end{matrix}\right.
\end{equation*}

donde, $x_{ij}$ corresponde a la j-ésima repetición del i-ésimo tratamiento; $\mu$ es el promedio general; $\tau_{i}$ corresponde al efecto del i-ésimo tratamiento ($\mu_{i} - \mu$); $\epsilon_{ij}$ es el error experimental.

La idea general del análisis de varianza consiste en separar la variación total en diferentes fuentes de variación: debido a los tratamientos y debido al error.

```{r, echo = FALSE, eval = TRUE, out.width = "44%", fig.align = "center"}

knitr::include_graphics("images/65va_imagen.png")  
```

```{r, echo = FALSE, eval = FALSE, out.width = "88%", fig.align = "center"}

knitr::include_graphics("images/anova.png")  
```

#### 7.1.1 Diseño completamente al azar

Este diseño se llama completamente al azar porque todos los grupos experimentales se distribuyen de manera completamente aleatoria, en relación a posibles efectos ambientales que pueden afectar la respuesta de algún grupo experimental. De esta manera, esos posibles efectos ambientales se pueden repartir equitativamente entre los tratamientos.

<div class="row">
  <div class="col-md-6" markdown="1">
```{r, echo = FALSE, eval = TRUE, out.width = "104%", fig.align = "center"}

knitr::include_graphics("images/68va_imagen.png")  
```
  </div>
  <div class="col-md-6" markdown="1">
```{r, echo = FALSE, eval = TRUE, out.width = "104%", fig.align = "center"}

knitr::include_graphics("images/69va_imagen.png")  
```
  </div>
</div>
...
knitr::include_graphics("images/67va_imagen.png")  
```
  </div>
  <div class="col-md-6" markdown="1">
```{r, eval = TRUE, echo = FALSE, message = FALSE}
...
```{r ejemplo_7_1_1_1, exercise = TRUE}
...
save(guanabana, file = "data_pkg/guanabana.rda", compress='xz')
```
...
tribble(
  ...
  download_this(
    output_name = 'guanabana',
    output_extension = '.xlsx',
    button_label = 'Descargar datos guanabana',
    button_type = 'warning',
    has_icon = TRUE,
    icon = 'fa fa-download'
  )
...
Los valores de la tabla anterior, se pueden describir con el siguiente modelo estadístico:

\begin{equation*}
y_{ij} = \mu + \tau_{i} + \epsilon_{ij}
\end{equation*}

La manera de probar si existen diferencias en el pH promedio de la pulpa de guanabana sometida a diferentes sistemas de empaque, consiste en contrastar la siguiente hipótesis:

\begin{equation*}
\begin{split}
&H_{0}: \mu_{e1} = \mu_{e2}  = \mu_{e3} = \mu_{e4} = \mu\\
&H_{a}: \mu_{i} \neq \mu_{i`}
\end{split}
\end{equation*}

Luego se realiza el análisis de varianza:

```{r ejemplo_7_1_1_1_b, exercise = TRUE}

anova_gua <- guanabana %>%
  mutate(
    ttos = as.factor(ttos)
    )
modelo <- aov(pH ~ ttos, data = anova_gua)
summary(modelo)
```

Se concluye que el pH promedio de la pulpa de guanaba si se ve afectada por el sistema de empaque.
