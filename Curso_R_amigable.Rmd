...
<details><summary>2.3 Operaciones con listas </summary> 
...
#### 5.2.1.10 Combinando funciones con `%>%`

Se pudo ver muchas de la funcionalidades interesantes y realmente utiles que ofrece el paquete `dplyr` y sus funciones. Sin embargo en la forma en como se explicaron, cada proceso requerira siempre guardarse por separado en objetos diferentes y hacerlo paso a paso:
...
Para solucionar este problema, el paquete `dplyr` proporciona el operador pipe (`%>%`). El `%>%` es un operador que permite encadenas funciones. Lo que hace es tomar la salida de una función y pasarla como entrada de la siguiente función. Esto ayuda enormemente a mejorar la legibilidad del código.
...
```{r ejemplo_5_2_1_10_1_b, exercise = TRUE, message = FALSE}
...
<!--
https://www.uv.es/pjperez/curso_R/tt_4_data_munging_v1.html#5_combinando_(joining)_df%E2%80%99s
-->
...
:::caution
Las funciones del paquete `dplyr` tienen muchas más posibilidades a las presentadas aquí. Si deseas ver un resumen de las posibilidades de este paquete, puedes mirar su hoja de trucos [aquí](https://github.com/Leo4Luffy/Un_curso_amigable_sobre_R/blob/master/Documentos/cheat_sheet_dplyr.pdf).
:::
